...
// --- ICONOS PREMIUM ---
const Icons = {
    Scan: () => <svg className="inline-size-32 block-size-32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 7V5a2 2 0 0 1 2-2h2" /><path d="M17 3h2a2 2 0 0 1 2 2v2" /><path d="M21 17v2a2 2 0 0 1-2 2h-2" /><path d="M7 21H5a2 2 0 0 1-2-2v-2" /><rect x="7" y="7" width="10" height="10" rx="2" /></svg>,
    Users: () => <svg className="inline-size-20 block-size-20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M22 21v-2a4 4 0 0 0-3-3.87" /><path d="M16 3.13a4 4 0 0 1 0 7.75" /></svg>,
    Plus: () => <svg className="inline-size-16 block-size-16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14" /><path d="M12 5v14" /></svg>,
    Check: () => <svg className="inline-size-16 block-size-16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12" /></svg>,
    ArrowRight: () => <svg className="inline-size-20 block-size-20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14" /><path d="M12 5l7 7-7 7" /></svg>,
    Sparkles: () => <svg className="inline-size-24 block-size-24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2Z" /></svg>
};
                ...
                {step === 'assign' && (
                    <div className="animate-in fade-in slide-in-from-bottom-8 duration-500 space-y-6">
                        ...
                        <div className="bg-gradient-to-br from-zinc-900 to-black border border-zinc-800 rounded-3xl p-8 text-center relative overflow-hidden shadow-2xl">
                            ...
                            {totals.unassigned > 0.01 ? (
                                ...
                                <div className="mt-4 inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-emerald-950/30 border border-emerald-900/50">
                                    <div className="inline-size-1.5 block-size-1.5 rounded-full bg-emerald-500"></div>
                            ...
                            <div className="flex gap-3 overflow-x-auto pb-4 no-scrollbar">
                                {people.map(p => (
                                    <div key={p.id} className="relative flex-shrink-0 group">
                                        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl block-size-14 px-5 flex items-center justify-center min-inline-size-[100px] shadow-sm group-hover:border-purple-500/50 transition-colors">
                                            <input
                                                value={p.name}
                                                onChange={(e) => updatePersonName(p.id, e.target.value)}
                                                className="bg-transparent text-white text-sm font-medium text-center inline-size-full focus:outline-none"
                                            />
                                        </div>
                                        {people.length > 1 && (
                                            <button onClick={() => setPeople(people.filter(x => x.id !== p.id))} className="absolute -inset-block-start-1 -inset-inline-end-1 inline-size-5 block-size-5 bg-zinc-800 rounded-full text-zinc-500 flex items-center justify-center text-[10px] opacity-0 group-hover:opacity-100 transition-opacity border border-zinc-700">✕</button>
                                        )}
                                    </div>
                                ))}
                                <button onClick={addPerson} className="flex-shrink-0 inline-size-14 block-size-14 rounded-2xl bg-zinc-900/50 border border-dashed border-zinc-700 flex items-center justify-center text-zinc-600 hover:text-zinc-400 hover:border-zinc-500 transition-all">
                                    <Icons.Plus />
                                </button>
                            </div>
                        </div>
                            ...
                            {items.map(item => {
                                ...
                                return (
                                    ...
                                    >
                                                    ...
                                                    {isAssigned ? (
                                                        <div className="flex -space-x-2">
                                                            {item.assignments.map(assignment => {
                                                                ...
                                                                return (
                                                                    <div key={assignment.personId} className="inline-size-6 block-size-6 rounded-full bg-purple-600 border-2 border-zinc-900 flex items-center justify-center text-[9px] font-bold text-white shadow-sm">
                                        ...
                                        {isAssigned && <div className="absolute inset-inline-start-0 inset-block-start-0 inset-block-end-0 inline-size-1 bg-gradient-to-b from-purple-500 to-pink-500"></div>}
                                    </div>
                                )
                            })}
                        </div>
                        ...
                        <button
                            ...
                            className={`inline-size-full py-4 rounded-2xl font-bold text-sm shadow-xl transition-all duration-300 flex items-center justify-center gap-2 mt-6 ${totals.unassigned < 0.1
                    ...
                    <div className="animate-in fade-in slide-in-from-right-8 duration-500 space-y-6">
                                    ...
                                    <div key={p.id} className="bg-zinc-900/50 p-6 rounded-3xl border border-zinc-800">
                                        <div className="flex items-center justify-between mb-3">
                                            <div className="flex items-center gap-4">
                                                <div className="inline-size-12 block-size-12 rounded-2xl bg-gradient-to-br from-purple-500 to-pink-600 flex items-center justify-center text-lg font-bold text-white shadow-lg shadow-purple-900/20">
                                                    {p.name.charAt(0)}
                                                </div>
                                                <div>
                        ...
                        <div className="flex flex-col gap-3 pt-4">
                            ...
                            <button
                                ...
                                className="inline-size-full py-4 rounded-2xl font-bold text-sm bg-zinc-900 text-zinc-400 hover:text-white border border-zinc-800 transition-colors"
                            >
                                Empezar de nuevo
                            </button>
                        </div>
